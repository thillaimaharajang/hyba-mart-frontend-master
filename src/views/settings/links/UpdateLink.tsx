...
const AddLink: React.FC = () => {
    ...
    const onSubmitUpdateLink = async (event: any) => {
        event.preventDefault()
        if (linksStore?.isValidCreateLinkForm()) {
            isValidForm = true;
            await LinksHelper(navigate).UpdateLink();
    ...
    return <PageTransition>
        <form onSubmit={onSubmitUpdateLink}>
            <div className="row mb-2">
                <div className="col-5">
                    <div className="col" style={{ fontWeight: '600', fontSize: '18px' }}>
                        <span style={{ color: '#000000' }}>Update Link:</span>
                    </div>
                </div>
            </div>
            ...
            <div className="row mt-3">
                <div className="col-4">
                    <Button htmlType='submit' className="custom-btn" type="primary" onClick={onSubmitUpdateLink}>UPDATE</Button>
                </div>
            </div>
