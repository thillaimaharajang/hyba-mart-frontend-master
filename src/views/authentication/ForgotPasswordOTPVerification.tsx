...
const ForgotPasswordOTPVerification = () => {
    ...
    const onSubmitForgotPasswordOTPVerification = async (event: any) => {
        event.preventDefault()
        if (authStore?.isValidOTPVerificationForm()) {
            isValidForm = true;
            await AuthHelper().ForgotPasswordOTPVerification(navigate);
        } else {
            isValidForm = false;
        }
    }
    ...
    const onResendOTP = async () => {
        await AuthHelper().ForgotPassword();
    }
    ...
    return <PageTransition>
        <RegistrationLayout header="OTP" subHeader="One Time Verification Password">
            ...
            <form onSubmit={onSubmitForgotPasswordOTPVerification} className="mt-3">
...
export default observer(ForgotPasswordOTPVerification);
